forward = function(x, orders, m, K, id0 = id0, n.em, em.iter, EM.iter, nk.min, tol, silent, Parallel, n.cores)
{
  p = ncol(x)
  n = nrow(x)
  s.id1 = inital_for_sid(p,m,K)
  s.id1.new = s.id1
  stop = numeric(K)
  x = x[, orders]
  ...
  while(m >0&&length(table(id0))==K){
    ...
    obj = select_add(x, m, K, id0 = id0, s.id = s.id1, BIC.all, EM.iter, nk.min, tol, silent, orders = orders, Parallel, n.cores)
    s.id1.new = obj$s.id
    id0 = obj$id
    BIC.all = obj$BIC
